...
! *
! *  Original Date: Jan 2018
! *
! *****************************************************************************/
!-----------------------------------------------------------------------------
!> Test for interpolation error.
!-----------------------------------------------------------------------------

!------------------------------------------------------------------------------
SUBROUTINE InterpolationTester( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
  USE DefUtils
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp) :: Norm, Err, sumx2, sumx
  INTEGER :: n, nb, nd, t, active
  INTEGER :: iter, maxiter, dim
  TYPE(Variable_t), POINTER :: ExactVar
  LOGICAL :: Found
!------------------------------------------------------------------------------
  ...
  ExactVar => VariableGet( Solver % Mesh % Variables,'Exact Field')
  IF( ASSOCIATED( ExactVar ) ) THEN
    sumx2 = SUM( ( ExactVar % Values - Solver % Variable % Values )**2 )
    sumx = SUM( ABS( ExactVar % Values - Solver % Variable % Values ) )
    n = SIZE( Solver % Variable % Values )
    sumx2 = ParallelReduction( sumx2 ) 
    sumx = ParallelReduction( sumx )
    n = NINT( ParallelReduction( 1.0_dp * n ) )
    Err = SQRT( sumx2 / n )

    CALL ListAddConstReal( Model % Simulation,'res: err-sq', Err ) 

    Err = sumx / n
    CALL ListAddConstReal( Model % Simulation,'res: err-abs', Err ) 
  END IF
...
!------------------------------------------------------------------------------
    ...
    DO t=1,IP % n
      ...
      rho = ListGetElementReal( TimeCoeff_h, Basis, Element, Found, GaussPoint = t ) 

      a(1) = ListGetElementReal( ConvVelo1_h, Basis, Element, Found, GaussPoint = t ) 
      a(2) = ListGetElementReal( ConvVelo2_h, Basis, Element, Found, GaussPoint = t ) 
      IF( dim == 3 ) THEN
        a(3) = ListGetElementReal( ConvVelo3_h, Basis, Element, Found, GaussPoint = t ) 
      END IF
        
      D = ListGetElementReal( DiffCoeff_h, Basis, Element, Found, GaussPoint = t ) 
      C = ListGetElementReal( ConvCoeff_h, Basis, Element, Found, GaussPoint = t ) 
      R = ListGetElementReal( ReactCoeff_h, Basis, Element, Found, GaussPoint = t ) 
...
!------------------------------------------------------------------------------
    ...
    DO t=1,IP % n
      ...
      ! Given flux:
      ! -----------
      F = ListGetElementReal( BCFlux_h, Basis, Element, Found, GaussPoint = t ) 

      ! Robin condition (C*(u-u_0)):
      ! ---------------------------
      C = ListGetElementReal( BCCoeff_h, Basis, Element, Found, GaussPoint = t ) 
      Ext = ListGetElementReal( BCExt_h, Basis, Element, Found, GaussPoint = t ) 
...
!------------------------------------------------------------------------------
END SUBROUTINE InterpolationTester
!------------------------------------------------------------------------------
